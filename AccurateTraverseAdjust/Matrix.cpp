...
//截取矩阵部分
CMatrix CMatrix::IntceptMatrix(CMatrix m0, int row_start, int col_start, int row_end, int col_end)
{
	if (m0.iCol > col_end|| m0.iRow>row_end)return false;
	...
	for (int i = row_start ; i < row_end; i++)
		for (int j = col_start ; j < col_end; j++)
			m1(i - row_start , j - col_start ) = m0(i, j);
	return m1;
}
